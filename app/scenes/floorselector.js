...
export default class FloorSelector extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.randDatas = getRandData(floorRand, this);
    ...
    if (checkedCode) {
      ...
      script = this.ev1data[select[0]][data.inputNo[0]] + ','
        + this.ev2data[select[1]][data.inputNo[1]] + ','
        + this.ev3data[select[2]][data.inputNo[2]] + ','
        + this.ev4data[select[3]][data.inputNo[3]];
    }

    //タグを置き換え : 置き換え後にタグが発生する場合があるのでdo-while
    do {
      script = tagReplace(script, this);
    } while (tagCheck(script));

    //RCRPに変換
    let commands = util.scriptsToEvents(script, checkedCode);
    commands = spCommandsFix(commands, checkedCode);
